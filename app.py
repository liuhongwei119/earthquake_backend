...
@app.route('/')
def index():
    return render_template('realtime_info.html', async_mode=socketio.async_mode)
...
def connect():
    ...
    with thread_lock:
        if thread is None:
            thread = socketio.start_background_task(target=imitate_mseed_task)
